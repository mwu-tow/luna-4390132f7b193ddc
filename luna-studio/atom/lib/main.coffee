...
module.exports = LunaStudio =
    ...
    activate: (state) ->
        ...
        @subs.add atom.commands.add 'atom-workspace', 'core:save', (e) => @handleSave(e)
    ...
    lunaOpener: (uri) ->
        if path.basename(uri) is lunaStudioFakePath
              new LunaStudioTab(null, nodeEditor)
        else if path.extname(uri) is '.luna'
              new LunaEditorTab(uri, codeEditor)

    handleSave: (e) ->
        activeItem = atom.workspace.getActivePaneItem()
        if (activeItem instanceof LunaEditorTab) or (activeItem instanceof LunaStudioTab)
            e.preventDefault()
            e.stopImmediatePropagation()
